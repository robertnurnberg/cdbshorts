...
int main(int argc, char **argv) {
    ...
    start_fen = "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq -"; // Startpos
    uint64_t start_depth = 0;
    if (argc != 1 && argc != 3) {
        std::cout << "Wrong count of arguments! Expect either none or fen with starting depth" << std::endl;
        return 1;
    }
    if (argc == 3) {
        start_fen = argv[1];
    ...
    std::priority_queue<std::pair<int, PackedBoard>, std::vector<std::pair<int, PackedBoard>>, std::greater<>> pq_progress;
    ...
    while (!to_visit.empty()) {
        ...
        while (!to_visit.empty()) {
            ...
            if (game_progress(current_progress_group, next_position) != ProgressOutcome::kSame) {
                break;
            }
        ...
        while (!pq_progress.empty()) {
            ...
            for (auto& cur_move : db_lookup) {
                ...
                if (visited.contains(packed_next_board) || added_by_dijkstra.contains(packed_next_board)) {
                    cur_position.unmakeMove(move);
                    continue;
                }

                // check if next position is in cdb to save memory
                int dist_to_root = cdbdirect_wrapper(handle, cur_position).back().second;
                ...
                if (game_progress(cur_position, current_progress_group) == ProgressOutcome::kLater) {
                    // this position comes later in the game, insert with the current depth unless the existing depth is better already
                    auto [to_visit_it, inserted] = to_visit.try_emplace(packed_next_board, cur_depth+1);
                    if (!inserted && to_visit_it->second > cur_depth+1) {
                        to_visit_it->second = cur_depth+1;
                    }
                }
                else {
                    // this position belongs to the current group, insert in the priority_queue
                    // to avoid accessive duplication store the pq insertion into a map so that it is only added once
                    // note that checking if a position is already in a pq is not fast so a second set is needed
        ...
        std::cout << "Progress group counter " << std::format("{:10}", group_cnt)
            << " subtree position count " << std::format("{:10}", total_num_positions)
            << " current group size " << std::format("{:10}", visited.size())
            << " to_visit size " << std::format("{:10}", to_visit.size())
            << " improved minply " << std::format("{:10}", improved_cdb_minply) << "\n";
        if (group_cnt % 100 == 0) {
            auto end_time = std::chrono::high_resolution_clock::now();
            auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(end_time-start_time).count();
            std::cout << "Resident memory: " << get_memory().second << " MB\n";
            std::cout << "time: " << std::format("{:10}", ms) << " ms     nps: " << std::format("{:10}", total_num_positions * 1000 / ms) << "\n";
